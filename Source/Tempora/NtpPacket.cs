...
namespace Tempora {
    internal class NtpPacket {
        ...
        private NtpPacket(byte[] bytes) {
            this.Bytes = bytes;
        }


        public static byte[] GetBytes(DateTime time, NtpPacket originPacket, DateTime arrivalTime) {
            var bytes = new List<byte>();

            var li = (int)NtpPacketLeapIndicator.None;
            var vn = (int)originPacket.VersionNumber;
            int mode;
            if (originPacket.Mode == NtpPacketMode.Client) {
                mode = (int)NtpPacketMode.Server;
            } else if (originPacket.Mode == NtpPacketMode.SymmetricActive) {
                mode = (int)NtpPacketMode.Server;
            } else {
                mode = (int)originPacket.Mode;
            }
            bytes.Add((byte)((li << 6) | (vn << 3) | (mode)));

            bytes.Add((byte)NtpPacketStratum.SecondaryReference3);
            bytes.Add(4); //PullInterval
            bytes.Add(0xfb); //Precision

            var rootDelay = BitConverter.GetBytes(1000);
            bytes.AddRange(new byte[] { rootDelay[3], rootDelay[2], rootDelay[1], rootDelay[0] });

            var rootDispersion = BitConverter.GetBytes(10000);
            bytes.AddRange(new byte[] { rootDispersion[3], rootDispersion[2], rootDispersion[1], rootDispersion[0] });

            bytes.AddRange(ASCIIEncoding.ASCII.GetBytes("LOCL"));

            bytes.AddRange(GetTimestamp(time.AddSeconds(-1)));
            bytes.AddRange(new byte[] { originPacket.Bytes[40], originPacket.Bytes[41], originPacket.Bytes[42], originPacket.Bytes[43], originPacket.Bytes[44], originPacket.Bytes[45], originPacket.Bytes[46], originPacket.Bytes[47] });
            bytes.AddRange(GetTimestamp(arrivalTime));
            bytes.AddRange(GetTimestamp(time));

            return bytes.ToArray();
        }


        public static NtpPacket ParseBytes(byte[] bytes) {
            var packet = new NtpPacket(bytes);

            packet.LeapIndicator = (NtpPacketLeapIndicator)(bytes[0] >> 6);
            packet.VersionNumber = (bytes[0] >> 3) & 0x07;
            packet.Mode = (NtpPacketMode)(bytes[0] & 0x07);

            packet.Stratum = (NtpPacketStratum)bytes[1];
            packet.PullInterval = new TimeSpan(0, 0, 0, (int)Math.Pow(2, bytes[2]));
            packet.Precision = new TimeSpan();

            packet.RootDelay = BitConverter.ToInt32(new byte[] { bytes[8], bytes[7], bytes[6], bytes[5] }, 0);

            packet.RootDispersion = BitConverter.ToInt32(new byte[] { bytes[12], bytes[11], bytes[10], bytes[9] }, 0); ;

            packet.ReferenceTimestamp = GetTimestamp(bytes, 16);
            packet.OriginateTimestamp = GetTimestamp(bytes, 24);
            packet.ReceiveTimestamp = GetTimestamp(bytes, 32);
            packet.TransmitTimestamp = GetTimestamp(bytes, 40);
        ...
        /// <summary>
        /// NTP version. Must be 3 (as per RFC 1305).
        /// </summary>
        ...
        /// </summary>
        public TimeSpan PullInterval { get; private set; }
        ...
        /// </summary>
        public int ReferenceClockIdentifier { get; private set; }
        ...
        /// </summary>
        public DateTime? OriginateTimestamp { get; private set; }
        ...
        private static byte[] GetTimestamp(DateTime? timestamp) {
            ...
            var n2 = (UInt32)Math.Min(Math.Max((time.Ticks - timeSec.Ticks) / 10000000.0 * 4294967296, 0), 4294967296);
    ...
    /// </summary>
    internal enum NtpPacketLeapIndicator {
        /// <summary>
        /// No warning.
        /// </summary>
        None = 0,
    ...
    /// </summary>
    internal enum NtpPacketStratum {
        /// <summary>
